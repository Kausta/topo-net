...
import phm
from phm.util.trainer import parse_optimizer, parse_scheduler
from phm.util.base import (
    Updateable,
    update_end_if_possible,
    update_if_possible,
)
from phm.util.config import parse_structured
from phm.util.misc import C, cleanup, get_device, load_module_weights
from phm.util.saving import SaverMixin
from phm.util.typing import *


class BaseTrainer(pl.LightningModule, Updateable, SaverMixin):
    @dataclass
    class Config:
    ...
    def load_weights(self, weights: str, ignore_modules: Optional[List[str]] = None):
        ...
        for dataloader in [self.trainer.train_dataloader, self.trainer.val_dataloaders, self.trainer.test_dataloaders, self.trainer.predict_dataloaders]:
            if dataloader is None:
                continue
            dataset = dataloader.dataset
            update_if_possible(dataset, epoch, global_step, on_load_weights=True)
    ...
    def configure_optimizers(self):
        ...
        if self.cfg.scheduler is not None:
            ret.update(
                {
                    "lr_scheduler": parse_scheduler(self.cfg.scheduler, optim, self.trainer),
                }
            )
        return ret
