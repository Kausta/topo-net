...
def parse_optimizer(config, model):
    if hasattr(config, "params"):
        ...
        phm.debug(f"Specify optimizer params: {config.params}")
    else:
...
def parse_scheduler(config, optimizer, trainer: pl.Trainer):
    ...
    elif config.name == "CosineWarmup":
        kwargs = {}
        if "min_lr" in config.args:
            kwargs["min_lr"] = config.args.min_lr
        # Get this property first as it may setup the train dataloader
        #  which is needed for the num_training_batches below 
        estimated_stepping_batches = trainer.estimated_stepping_batches
        scheduler = {
            "scheduler": pnn.get_cosine_schedule_with_warmup(
                optimizer, 
                config.args.warmup_epochs * trainer.num_training_batches, 
