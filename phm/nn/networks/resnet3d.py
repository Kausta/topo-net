...
@phm.register("network-resnet3d")
class Resnet3D(BaseNetwork):
    @dataclass
    class Config(BaseNetwork.Config):
        model: str = "resnet18"  # resnet18 | resnet50
        conv: str = "ACSConv" # ACSConv | Conv2_5d | Conv3d
        pretrained_3d: str = "i3d"
    ...
    def configure(self) -> None:
        ...
        assert model_str in ["resnet18", "resnet50"], f"Unknown model {self.cfg.model}"

        in_ch = self.cfg.in_ch
        out_ch = self.cfg.out_ch

        if model_str == 'resnet18':
            model = ResNet18(in_channels=in_ch, num_classes=out_ch)
        elif model_str == 'resnet50':
            model = ResNet50(in_channels=in_ch, num_classes=out_ch)
        else:
            raise NotImplementedError

        conv = self.cfg.conv
        assert conv in ["ACSConv", "Conv2_5d", "Conv3d"]
        pretrained_3d = self.cfg.pretrained_3d
        ...
        if conv=='Conv3d':
            if pretrained_3d == 'i3d':
                model = Conv3dConverter(model, i3d_repeat_axis=-3)
            else:
