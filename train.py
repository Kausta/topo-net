...
def main(args, extras) -> None:
    ...
    import phm
    from phm.trainers.base import BaseTrainer
    from phm.util.callbacks import (
        CodeSnapshotCallback,
        ConfigSnapshotCallback,
        CustomProgressBar,
        ProgressCallback,
    )
    from phm.util.config import ExperimentConfig, load_config
    from phm.util.misc import get_rank
    from phm.util.typing import Optional
    ...
    dm = phm.find(cfg.data_type)(cfg.data)
    system: BaseTrainer = phm.find(cfg.trainer_type)(
    ...
    if args.train:
        ...
        if args.gradio:
            callbacks += [
                ProgressCallback(save_path=os.path.join(
                    cfg.trial_dir, "progress"))
            ]
        else:
    ...
    if args.train:
        ...
        rank_zero_only(
            lambda: os.makedirs(os.path.join(
                cfg.trial_dir, "tb_logs"), exist_ok=True)
        )()
        loggers += [
    ...
    def set_system_status(system: BaseTrainer, ckpt_path: Optional[str]):
    ...
    if args.train:
        ...
        if cfg.test_best_val:
            trainer.test(system, datamodule=dm, ckpt_path="best")
        if args.gradio:
